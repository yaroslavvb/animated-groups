...
four traditional colour types each cover two fixed-time records with
complementary forward skips. The concise
[notation note](docs/orbifold_notation.html) lists those four cases and compares
the convention with Conway et al.'s fibrifold notation. The catalog fixes
`A = 0`, `B = 1/N`, and so on, and writes one forward tick as
`C_N = (ABC…)`. A generator with time skip `+k/N` acts by `C_N^k`; the identity
and powers with several disjoint cycles are still elements of this one cyclic
image. No operation in these forward groups reverses time.
