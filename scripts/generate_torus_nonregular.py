...
def cosets(H):
    """Left cosets gH, ordered by the colour each sends A to."""
    seen, out = set(), []
    for g in GAMMA:
...
def case(H, key, title, note):
    blocks = cosets(H)
    K = core(H)
    stab = {i: frozenset(g for g in GAMMA if g[i] == i) for i in range(4)}
    ...
    coset_rows = "".join(
        ...
        f"<td>sends <strong>A</strong> &rarr; <strong>{NAMES[b[0][0]]}</strong></td></tr>"
        for i, b in enumerate(blocks)
    )
    return f"""
    <section class="case" id="case-{key}">
      <h3>H = <code>{{{escape(names)}}}</code></h3>
